...
If we look at the difference between the Covid and Excess deaths, and plot 
that against Excess deaths, we see the same effect, with an even better
fit - a very tight point cloud. As the proportion - these are deaths per
capita - grows, the disparity between covid deaths and excess deaths grows.
...
foo %>% 
  ggplot(aes(x=Covid_percap, y=Excess_minus_Covid)) +
  geom_point()+
  geom_smooth(method=lm)+
  annotate("text", x=200, y=500, 
           label=(paste0("slope==",                      signif(coef(lm(foo$Excess_minus_Covid~foo$Covid_percap))[2],3))),
           parse=TRUE)+
  labs(title="Covid Deaths vs. Difference (Excess-Covid) (per 100,000)",
       subtitle="By County",
       x="Covid Deaths per 100,000",
       y="Excess - Covid Deaths per 100,000")
