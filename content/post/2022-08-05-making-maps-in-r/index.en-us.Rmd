...
```{r basemaps OpenStreetMap, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r basemaps basemaps, warning=FALSE, message=FALSE, cache=TRUE}
########  package: basemaps
...
```{r basemaps basemapR, warning=FALSE, message=FALSE, cache=TRUE}
########  package: basemapR
# Produces a ggplot object
Basemap_3 <- basemapR::base_map(bbox, basemap="mapnik", increase_zoom=2)
```

```{r basemaps ggspatial, warning=FALSE, message=FALSE, cache=TRUE}
########  package: ggspatial
# Produces a ggplot object
Basemap_4 <- ggplot(df_sf) + ggspatial::annotation_map_tile(zoomin=-1) 
```

```{r basemaps plots, warning=FALSE, message=FALSE}
...
```{r size table2, echo=FALSE, results='asis'}
...
```{r basemaps vector plot, warning=FALSE, message=FALSE}
...
interp_IDW <- predict(fit_IDW, grd_sf)
...
Validate <- sf::st_as_sf(gstat::gstat.cv(fit_IDW, debug.level=0))
...
for (power in (1:6)*0.5) {
  ...
  foo <- sf::st_as_sf(gstat::gstat.cv(my_fit)) %>% 
    rename(Observed=observed, Predicted=var1.pred ) %>% 
    mutate(power=power) 
...
Now let's transfporm our contours to lat-long and plot
