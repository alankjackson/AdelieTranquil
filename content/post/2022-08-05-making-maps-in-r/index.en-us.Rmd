...
```{r basemaps OpenStreetMap, warning=TRUE, message=TRUE}

# Produces an OpenStreetMap object (and caches it)
Base_osm <- xfun::cache_rds({
...
```{r basemaps basemaps, warning=TRUE, message=TRUE}
# Produces a ggplot object (and caches it)
Base_basemaps <- xfun::cache_rds({
...
```{r basemaps basemapR, warning=TRUE, message=TRUE}
# Produces a ggplot object
Base_basemapR <- xfun::cache_rds({
...
```{r basemaps ggspatial, warning=TRUE, message=TRUE}
# Produces a ggplot object
Base_ggspatial <- xfun::cache_rds({
...
```{r basemaps_plots, warning=TRUE, message=TRUE}
...
```{r basemaps_2, warning=TRUE, message=TRUE}
...
```{r basemaps_vector_plot, warning=TRUE, message=TRUE}
...
was to place the original data on top of the colored contour fill, and give both the same
...
Validate %>% 
  ...
  #scale_color_gradientn(colors=rainbow(5), limits=c(-2,2)) 
...
<!--  ```{r temp, messages=FALSE, echo=FALSE} -->
...
<!-- interp_IDW <- predict(fit_IDW, df_out, debug.level = 0) -->
...
topographies. Folded systems we expect to be smoothly varying with no "bulls-eyes".
...
Now let's transform our contours to lat-long and plot
...
ggplot() +
  ...
  #         aes(fill=focal_mean), alpha=0.4, na.value="transparent") +
  # colorspace::scale_fill_discrete_sequential(name = "Inches Rain",
  #                                            palette="Blues 3") +
  geom_sf(data=Contours_LL, color="black") +
  #scale_color_gradientn(colors=rainbow(5), limits=c(0,3)) +
  #scale_fill_gradientn(colors=rainbow(5), limits=c(0,3), na.value="transparent") +
  #scale_fill_gradientn(palette="Blue 3", limits=c(0,3), na.value="transparent") +
...
Poly_fit.1 <- sf::st_as_sf(bind_cols(grd_df,data.frame(var1.pred=predict(lm.1, grd_df))), crs=localcrs, coords=c("X", "Y"))

#   Convert to a stars object 
Poly_fit_star.1 <- stars::st_rasterize(Poly_fit.1 %>% dplyr::select(Rain=var1.pred, geometry))
...
Poly_fit.2 <- sf::st_as_sf(bind_cols(grd_df,data.frame(var1.pred=predict(lm.2, grd_df))), crs=localcrs, coords=c("X", "Y"))

#   Convert to a stars object 
Poly_fit_star.2 <- stars::st_rasterize(Poly_fit.2 %>% dplyr::select(Rain=var1.pred, geometry))
...
#   First remove that fit from the input data
...
dat.fit  <- gstat::fit.variogram(var.smpl, fit.ranges = TRUE, fit.sills = TRUE,
                                 debug.level = 0,
                          gstat::vgm(psill=NA, model="Exp", range=NA, nugget=0))

##plot(var.smpl, dat.fit, xlim=c(0,30000))

ggplot(var.smpl, aes(x = dist, y = gamma)) +
     geom_point() +
     geom_line(data = gstat::variogramLine(dat.fit, maxdist = max(var.smpl$dist))) +
     labs(title="Variogram for detrended data")

#   We have our model, so now let's use it

# foo <- bind_cols(df_xy$Rain, as_tibble(sf::st_coordinates(df_xy))) %>% 
#   rename(Rain = 1)
# 
# Krigged_grid <- gstat::krige(Rain ~ X + Y,
#                              foo,
#                              grd_sf,
#                              dat.fit)
...
ggplot() +
  ...
  #geom_sf(data=sf::st_as_sf(Final_krige_star), aes(color=value), size=5) +
  geom_sf(data=df_xy, aes(color=Rain), size=5) +
  scale_color_gradientn(colors=rainbow(5), limits=c(0,3)) +
  scale_fill_gradientn(colors=rainbow(5), limits=c(0,3)) +
  geom_sf(data=df_xy, color="Black", size=1) +
...
foo_terra <- terra::rast(Final_krige_star) 
foo_sm <- terra::focal(terra::focal(foo_terra, w=3, fun=median), w=5, fun=mean)
...
One really nice feature of Kriging is that it automatically gives an error estimate,
so you can easily get a feel for how robust the fitting is. So it is clear that the 
variance is pretty small around each of the input data locations, and can get quite
a bit larger as you interpolate away from data. This reflects the rather large variation in
the input data, showing that away from control, all bets are off.
...
#   Convert to a stars object so we can use the contouring in stars
Spline_stars <- stars::st_rasterize(Spline_grid %>% dplyr::select(value=Rain, geometry))
#   Make some pretty breaks for contour intervals based on the data
brks <- classInt::classIntervals(pretty(n=10, x=as.vector(Spline_stars[[1]])), n=8)
#   Create contour lines
Contours_Spline <- stars::st_contour(Spline_stars, contour_lines=TRUE, breaks=brks$brks)
#   Plot to see what it all looks like
#   and our final plot in X-Y space
ggplot() +
  geom_sf(data=Spline_grid, aes(color=Rain), size=5) +
  geom_sf(data=df_xy, aes(color=Rain), size=5) +
  geom_sf(data=df_xy, color="Black", size=1) +
  scale_color_gradientn(colors=rainbow(5)) +
  geom_sf(data=Contours_Spline, color="red") 
...
#   Make some pretty breaks for contour intervals based on the data
brks <- classInt::classIntervals(pretty(n=10, x=as.vector(TPS_stars[[1]])), n=8)
#   Create contour lines
Contours_TPS <- stars::st_contour(TPS_stars, contour_lines=TRUE, breaks=brks$brks)
#   Plot to see what it all looks like
#   and our final plot in X-Y space
ggplot() +
  #stars::geom_stars(data=rename(TPS_stars, Rain=layer) %>% 
  #                   mutate(Rain = if_else(between(Rain, 0, 3), Rain, NULL))) +
  stars::geom_stars(data=rename(TPS_stars, Rain=layer)) +  
  geom_sf(data=df_xy, aes(color=Rain), size=5) +
  geom_sf(data=df_xy, color="Black", size=1) +
  scale_fill_gradientn(colors=rainbow(5), limits=c(0,3)) +
  scale_color_gradientn(colors=rainbow(5), limits=c(0,3)) +
  geom_sf(data=Contours_TPS, color="red") 
```
