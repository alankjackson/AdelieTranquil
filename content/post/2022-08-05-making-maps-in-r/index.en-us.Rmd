...
#    Two ways to create a grid definition - note that the first gives corner coordinates.

grd_template <- expand.grid(
  X = seq(from = bbox_xy["xmin"], to = bbox_xy["xmax"], by = 2000),
  Y = seq(from = bbox_xy["ymin"], to = bbox_xy["ymax"], by = 2000) # 2000 m resolution
)
...
#     Create interpolators and interpolate to grid
...
fit_IDW <- gstat::gstat( # The setup here is quite similar to NN
  formula = Rain ~ 1,
  data = df_xy, # using {sf} and converting to {sp}
  nmax = 10, nmin = 3,
...
Contours_IDW <- stars::st_contour(interp_IDW_stars, contour_lines=TRUE)
...
foo_sm <- terra::focal(terra::focal(foo_terra, w=3, fun=median), w=3, fun=mean)

#  One more step - let's smooth the contours
Contours_sm <- stars::st_contour(stars::st_as_stars(foo_sm), contour_lines=FALSE)
#Contours_sm <- stars::st_contour(foo_sm, contour_lines=TRUE)
...
#foo_sm_ll <- sf::st_transform(stars::st_as_stars(foo_sm), crs=googlecrs)
#Contours_sm <- stars::st_contour(foo_sm_ll, contour_lines=TRUE)
#Contours_sm2 <- smoothr::smooth(Contours_sm, method="ksmooth", smoothness=2)
