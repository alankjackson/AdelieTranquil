...
(that is X-Y coordinates) is OpenStreetMap. 
...
the various vector layers are quite large, but the final map is quite small, as can
be seen in the table below.

```{r size table, echo-=FALSE, results='asis'}
Basemap_size <- tribble( ~Package, ~Size_MB,
                         "Big Streets", 36.8,
                         "Medium Streets", 215.3,
                         "Rivers", 23.3,
                         "Lakes", 45.9,
                         "ggplot object", 0.0057)
knitr::kable(Basemap_size, caption="Sizes in MB for different map objects")
```
...
big_streets <- 
  ...
  osmdata::trim_osmdata(tmaptools::bb(bbox, output="matrix")) # Trim to AOI
...
river <- tmaptools::bb(bbox, output="matrix") %>%
  ...
  osmdata::osmdata_sf() %>% 
  osmdata::trim_osmdata(tmaptools::bb(bbox, output="matrix"))
