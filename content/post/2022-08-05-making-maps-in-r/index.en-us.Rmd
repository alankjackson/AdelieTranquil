...
grd_template3 <- terra::rast(xmin=bbox_xy["xmin"]-500, xmax=bbox_xy["xmax"]+1000,
...
grd_template4 <- raster::raster(xmn=bbox_xy["xmin"]-500, xmx=bbox_xy["xmax"]+1000,
...
Poly_fit.2 <- sf::st_as_sf(bind_cols(grd_template,data.frame(var1.pred=predict(lm.2, grd_template))), crs=localcrs, coords=c("X", "Y"))
...
interp_TPS <- raster::interpolate(grd_template4, fit_TPS)

#   Convert to spatraster
##interp_TPS <- terra::rast(interp_TPS)
...
#   and our final plot in X-Y space
ggplot() +
  geom_sf(data=sf::st_as_sf(TPS_stars), aes(color=layer), size=5) +
  geom_sf(data=df_xy, aes(color=Rain), size=5) +
  geom_sf(data=df_xy, color="Black", size=1) +
  scale_color_gradientn(colors=rainbow(5)) +
