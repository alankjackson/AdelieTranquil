...
Note that when smoothing, we lose the data on the edges due to the 1/2 width of the
...
#   Let's do a 5x5 median followed by a 7x7 mean
...
ggplot() +
  ...
  geom_sf(data=Contours_sm2, color="blue", alpha=0.4) 
...
#   Convert to a stars object 
Poly_fit_star.2 <- stars::st_rasterize(Poly_fit.2 %>% dplyr::select(value=var1.pred, geometry))
...
trend <- bind_cols(predict(lm.1, as_tibble(sf::st_coordinates(df_xy))), 
...
Krigged_grid <- gstat::krige(Detrended ~ 1,
                             ...
                             grd_sf,
                             dat.fit)
Krigged_star <- stars::st_rasterize(Krigged_grid %>% dplyr::select(value=var1.pred, geometry))
Krigged_starvar <- stars::st_rasterize(Krigged_grid %>% dplyr::select(value=var1.var, geometry))
